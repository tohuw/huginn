...
Extracted from ``huginn.sources.transcript`` unchanged -- issue #42. This module
had no daemon or config imports, which is why it was the highest-value piece to
share: its awkward edges (a single JSONL record larger than the read window, a
file truncated below the stored offset, a partial trailing line carried across
reads) are expensive to get right twice. ``Tail``, ``ClaudeAnalyzer`` and
``CodexAnalyzer`` are covered by this package's stability promise; the
underscored helpers below are not.
"""
...
# Deliberately narrow -- issue #42 asked for a stability guarantee, not a bigger
# API. ATTACH_WINDOW/MAX_READ/MAX_ATTACH_LINE are listed because they are the
# tuning knobs a consumer needs to reason about (and test against) the tailer's
# bounds. Everything underscored above is an implementation detail with no
# promise attached.
__all__ = [
    "ATTACH_WINDOW",
    "MAX_ATTACH_LINE",
